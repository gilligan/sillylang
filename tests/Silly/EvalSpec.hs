  ...
  describe "Eval" $ do
    ...
    describe "SillyInterpreter" $ do
      ...
      describe "if clauses" $ do
        it "runs the statements when 'cond' is truthy" $ do
          res <- runWithInitialState $ do
            runStmt $ IfCond (LitBool True) [VarStmt "cond-was-true" (LitBool True)]
            lookupRef "cond-was-true"
          res `shouldBe` Right (Just $ BoolValue True)

      describe "if clauses" $ do
        it "does not run the statements when 'cond' is truthy" $ do
          res <- runWithInitialState $ do
            runStmt $ IfCond (LitBool False) [VarStmt "cond-was-true" (LitBool True)]
            lookupRef "cond-was-true"
